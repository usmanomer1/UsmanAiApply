/*
  ...
  1. New Tables
    - `ai_token_usage`
      ...
      - `total_tokens` (integer, default 0, auto-calculated)
      - `max_tokens_requested` (integer, default 0)
  ...
  2. Security
    ...
    - Add policies for system to insert usage data

  3. Functions
    - `calculate_total_tokens()` - Auto-calculate total from prompt + completion tokens
    - `get_user_monthly_ai_tokens()` - Get current month usage for a user
    - `can_user_make_ai_request()` - Check if user can make request (subscription + token limits)

  4. Performance
    - Indexes on user_id, created_at, operation_type, and composite user_month
*/
